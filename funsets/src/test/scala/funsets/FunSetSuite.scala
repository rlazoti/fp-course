...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains all elements in this set but not in that set") {
    new TestSets {
      ...
      assert(contains(s, 3), "s contains 2")
      assert(!contains(s, 1), "s not contains 1")
    }
  }
  ...
  test("filter") {
    new TestSets {
      ...
      val s6 = union(union(union(union(union(s1, s3), singletonSet(4)), singletonSet(5)), singletonSet(7)), singletonSet(1000))
      val s7 = filter(s6, x => x < 5)

      assert(!contains(s7, 7))
      assert(!contains(s7, 1000))
    }
  }
  ...
  test("map") {
    new TestSets {
      ...
      assert(contains(s5, 6), "s5 contains 3")
      assert(contains(s5, 9), "s5 contains 3")
    }
  }
